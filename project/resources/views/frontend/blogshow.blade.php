                        ...
                        <div class="widget border-0 py-0 widget_recent_entries">
                            ...
                            <ul>
                                @foreach (App\Models\Blog::latest()->limit(4)->get() as $reblog)
                                    <li>
                                        <a
                                            href="{{ route('front.blogshow', $reblog->slug) }}">{{ mb_strlen($reblog->title, 'UTF-8') > 45 ? mb_substr($reblog->title, 0, 45, 'UTF-8') . '..' : $reblog->title }}</a>
                                        <span
                                            class="post-date">{{ date('M d - Y', strtotime($reblog->created_at)) }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div class="single-post">
                        <div class="single-post-title">
                            ...
                            <div class="post-meta mb-4">
                                <a href="#"><i class="flaticon-user-silhouette flat-mini"></i>
                                    <span>{{ __('By Admin') }}</span></a>
                                <a href="#"><i class="flaticon-calendar flat-mini"></i>
                                    <span>{{ date('M d - Y', strtotime($blog->created_at)) }}</span></a>
                                <a href="#"><i class="flaticon-like flat-mini"></i> <span>{{ $blog->views }}
                                        {{ __('View(s)') }}</span></a>
                                <span><i class="flaticon-document flat-mini text-primary"></i> <a
                                        href="#"><span>{{ __('Source') }} : </span></a><a
                                        href="#"><span>{{ $blog->source }}</span></a></span>
                            </div>
                        </div>
                        <div class="img">
                            <img data-src="{{ asset('assets/images/blogs/' . $blog->photo) }}" class="img-fluid lazy"
                                alt="">
                        </div>
                        <div class="post-content pt-4 mb-5">
                            <p>{!! clean($blog->details, ['Attr.EnableID' => true]) !!}</p>
                        </div>
                            ...
                            <div class="comments">
                                <div id="disqus_thread">
                                    ...
                                    <noscript>{{ __('Please enable JavaScript to view the') }} <a
                                            href="https://disqus.com/?ref_noscript">{{ __('comments powered by Disqus.') }}</a></noscript>
                                </div>
                            </div>
                        @endif
                        {{-- DISQUS ENDS --}}
